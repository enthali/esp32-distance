 ...
 * - Loads configuration from config_manager (same as hardware)
 *
...
typedef struct
{
    gpio_num_t trigger_pin;           ///< Ignored in simulator
    gpio_num_t echo_pin;              ///< Ignored in simulator
    uint32_t measurement_interval_ms; ///< Measurement interval
    uint32_t timeout_ms;              ///< Ignored in simulator
    int16_t temperature_c_x10;        ///< Ignored in simulator
    uint16_t smoothing_factor;        ///< Ignored in simulator
...
esp_err_t distance_sensor_init(gpio_num_t trigger_pin, gpio_num_t echo_pin)
{
    ...
    ESP_LOGI(TAG, "Initializing distance sensor simulator (loading config from NVS)...");
    
    // Store hardware pins (ignored in simulator but logged for consistency)
    sensor_config.trigger_pin = trigger_pin;
    sensor_config.echo_pin = echo_pin;
    
    ESP_LOGI(TAG, "Hardware pins: Trigger=GPIO%d, Echo=GPIO%d (ignored in simulator)", 
    ...
    // Other config values (timeout, temperature, smoothing) are ignored in simulator
    // but we read them for consistency with hardware version
    int32_t sens_timeout_ms = 0;
    config_get_int32("sens_timeout_ms", &sens_timeout_ms);
    sensor_config.timeout_ms = (uint32_t)sens_timeout_ms;
    
    int16_t temp_c_x10 = 0;
    config_get_int16("temp_c_x10", &temp_c_x10);
    sensor_config.temperature_c_x10 = temp_c_x10;
    
    int16_t smooth_factor = 0;
    config_get_int16("smooth_factor", &smooth_factor);
    ...
    ESP_LOGI(TAG, "  [Simulator: timeout, temperature, smoothing ignored]");
